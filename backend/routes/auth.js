...
router.post('/register/doctor', async (req,res)=>{
  try {
    ...
    const hash = await bcrypt.hash(password,10);
    const user = await User.create({ email, passwordHash: hash, role: 'doctor', aabhaId, aadharNo });
    ...
    const doc = await Doctor.create({
      ...
      nmcRegNo
        ...
        res.status(201).json({ message: 'Doctor registered', userId: user._id, role: 'doctor' });
  } catch (err) { 
    console.error(err); 
    res.status(500).json({ error: 'Server error' }); 
  }
...
router.post('/register/insurer', async (req,res)=>{
  try {
    ...
    const hash = await bcrypt.hash(password,10);
    const user = await User.create({ email, passwordHash: hash, role: 'insurer', aabhaId, aadharNo });
    ...
    res.status(201).json({ message: 'Insurer registered', userId: user._id, role: 'insurer' });
  } catch (err) { 
    console.error(err); 
    res.status(500).json({ error: 'Server error' }); 
  }
