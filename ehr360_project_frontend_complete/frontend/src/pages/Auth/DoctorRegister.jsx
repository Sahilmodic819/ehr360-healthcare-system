...
export default function DoctorRegister(){
  ...
  const submit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(import.meta.env.VITE_API_BASE + '/api/auth/register/doctor', form);
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <label>NMC Registration No.</label><input className="input" name="nmcRegNo" value={form.nmcRegNo} onChange={e=>setForm({...form,nmcRegNo:e.target.value})} />
